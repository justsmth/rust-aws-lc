    ...
    S::Response: AsyncRead + AsyncWrite + Connection + Unpin + Debug + Sync + Send + 'static,
{
    ...
    fn call(&mut self, uri: Uri) -> Self::Future {
        ...
        let f = async {
            ...
            // it (otherwise, aws-lc will fail to parse the host as an IP address, eventually
            ...
            let stream = tokio_aws_lc::connect(config, host, conn).await?;
