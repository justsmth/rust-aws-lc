use aws_lc::ssl::{SslAcceptor, SslConnector, SslFiletype, SslMethod};
use futures::future;
...
async fn google() {
    ...
    let config = SslConnector::builder(SslMethod::tls())
        ...
        .configure()
        .unwrap();
    let mut stream = tokio_aws_lc::connect(config, "google.com", stream)
        .await
        .unwrap();
...
fn create_server() -> (
    ...
    let server = async move {
        ...
        tokio_aws_lc::accept(&acceptor, stream).await
    };
...
async fn server() {
    ...
    let client = async {
        ...
        let stream = TcpStream::connect(&addr).await.unwrap();
        let mut stream = tokio_aws_lc::connect(config, "localhost", stream)
            .await
            .unwrap();
...
async fn handshake_error() {
    ...
    let client = async {
        ...
        let err = tokio_aws_lc::connect(config, "localhost", stream)
            .await
            .unwrap_err();
